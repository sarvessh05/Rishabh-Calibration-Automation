...
def calculate_errors(res: Dict, applied_inputs: Dict) -> None:
    ...
    for param, vals in res["params"].items():
        ...
        if param == "watt_sum":
            vals["%error"] = None
            vals["pass_fail"] = "PASS" if 1138 <= reading <= 1161 else "FAIL"
        elif param == "var_sum":
            vals["%error"] = None
            vals["pass_fail"] = "PASS" if 1970 <= reading <= 2009 else "FAIL"
...
def save_results(results: List[Dict]):
    ...
    out_file = os.path.join(config.RESULTS_DIR, "3WS2.json")
