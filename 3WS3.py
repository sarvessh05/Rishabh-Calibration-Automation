...
def calculate_errors(res: Dict, applied_inputs: Dict) -> None:
    ...
    for param, vals in res["params"].items():
        ...
        if param == "watt_sum":
            vals["%error"] = None
            vals["pass_fail"] = "PASS" if 455 <= reading <= 464 else "FAIL"
        elif param == "var_sum":
            vals["%error"] = None
            vals["pass_fail"] = "PASS" if -2 <= reading <= 2 else "FAIL"
...
def save_results(results: List[Dict]):
    ...
    out_file = os.path.join(config.RESULTS_DIR, "3WS3.json")
