...
def calculate_errors(res: Dict, applied_inputs: Dict) -> None:
    ...
    for param, vals in res["params"].items():
        ...
        if param == "watt_sum":
            vals["%error"] = None
            vals["pass_fail"] = "PASS" if 2277 <= reading <= 2323 else "FAIL"
        elif param == "var_sum":
            vals["%error"] = None
            vals["pass_fail"] = "PASS" if -3 <= reading <= 3 else "FAIL"
...
def save_results(results: List[Dict]):
    ...
    out_file = os.path.join(config.RESULTS_DIR, "3WS1.json")
...
def main(angle: int = 0):
    ...
    if angle == 0:
        applied_inputs = {
            "watt_sum": 2300,"var_sum": 0,
            "voltage_L1": 398.36, "voltage_L2": 0.0, "voltage_L3": 398.36,
            "current_L1": 5.0, "current_L2": 5.0, "current_L3": 5.0,
            "frequency": 50.0
        }
    else:
        raise ValueError("Unsupported angle. Use 0 .")
